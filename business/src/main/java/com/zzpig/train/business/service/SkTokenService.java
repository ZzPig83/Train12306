...
import com.zzpig.train.business.mapper.cust.SkTokenMapperCust;
...
import com.zzpig.train.business.mapper.SkTokenMapper;
...
public class SkTokenService {
    ...
    public void genDaily(Date date, String trainCode) {
        ...
        int seatCount = dailyTrainSeatService.countSeat(date, trainCode, null); // 类型设置为空，获取全部座位数
        LOG.info("车次【{}】座位数：{}", trainCode, seatCount);
        long stationCount = dailyTrainStationService.countStation(date, trainCode);
        LOG.info("车次【{}】车站数：{}", trainCode, stationCount);

        // 5/6需要根据实际卖票比例来定，一趟火车最多可以卖(seatCount * stationCount)张火车票
    ...
    public PageResp<SkTokenQueryResp> queryList(SkTokenQueryReq req) {
        SkTokenExample skTokenExample = new SkTokenExample();
        skTokenExample.setOrderByClause("date desc");
    ...
    public boolean validSkToken(Date date, String trainCode, Long memberId) {
        ...
        if (!env.equals("dev")) {
            // 先获取令牌锁，再校验令牌余量，防止机器人抢票，lockKey就是令牌，用来表示【谁能做什么】的凭证
        ...
        if (skTokenCount != null) {
            ...
            LOG.info("缓存中没有该车次令牌的大闸的key: {}", skTokenCountKey);
            // 检查是否还有令牌
            SkTokenExample skTokenExample = new SkTokenExample();
            skTokenExample.createCriteria().andDateEqualTo(date).andTrainCodeEqualTo(trainCode);
...
//            return false;
//        }
        // 最后不要释放锁，因为要锁定5秒钟，实现同一用户5秒之内不能再次抢票
    }
}
